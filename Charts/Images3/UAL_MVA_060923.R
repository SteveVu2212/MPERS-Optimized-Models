...
chart1 <- read_xlsx(paste0(file_path, data_file), sheet = "UAL-MVA")

chart1 |>
  dplyr::filter(Year >= 2020) |>
  dplyr::mutate(Year = as.character(Year)) |>
  mutate(SQ_w_Assumed_ROA = SQ_w_Assumed_ROA / 1e3) |>
  mutate(ADC_w_Assumed_ROA = ADC_w_Assumed_ROA / 1e3) |>
  mutate(SQ_w_RecurringRecession_ROA = SQ_w_RecurringRecession_ROA / 1e3) |>
  mutate(ADC_w_RecurringRecession_ROA = ADC_w_RecurringRecession_ROA / 1e3) |>
  ...
  e_title(
    text = "Unfunded Liabilities (MVA Basis) ($ Billions)",
    left = "0%",
    top = "0%",
